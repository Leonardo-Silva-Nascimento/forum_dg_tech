<template>
  ...
  <Nav class="mt-4 text-sm text-gray-400 list-none bg-white p-3 px-10 rounded-sm flex space-x-10 shadow-sm">
    ...
    <button class="font-bold" :class="{'text-primary': currentNav == 2}" @click="currentNav = 2">
      Permissões
    </button>
  </Nav>
  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      ...
      <div class="flex space-x-5 mt-8">
        ...
        <Link :href="route('list.users')" as="button" type="button" class="p-2 rounded-md bg-third text-white px-6 text-sm font-medium">
        Voltar
        </Link>
      </div>
    </section>
